...
class featureExtracter(nn.Module):
    def __init__(self, seqL=5):
        ...
        self.transformer_encoder2 = torch.nn.TransformerEncoder(
            encoder_layer2, num_layers=1
        )
        ...
        self.net_vlad = NetVLADLoupe(
            feature_size=512,
            max_samples=int(900 * self.seqL),
            cluster_size=64,  # before 11.12 --- 64
            output_dim=256,
            gating=True,
            add_batch_norm=False,  # output_dim=512
            is_training=True,
        )
