...
_NUMPY_DTYPE_TO_POINT_FIELD = {
    numpy_dtype: point_field_type for point_field_type,
    numpy_dtype in _POINT_FIELDS_AND_NUMPY_DTYPES_PAIRS
}
    ...
    for f in fields:
        while offset < f.offset:
            ...
            np_dtype_list.append(
                ('%s%d' % (_DUMMY_FIELD_PREFIX, offset), np.uint8))
            offset += 1
...
def ros_point_cloud_to_lidar(
    ...
    numpy_point_cloud = ros_point_cloud_to_numpy_point_cloud(
        point_cloud).astype(np.float32)[:, :3]
    if timestamp is None:
        timestamp = time()
    return Lidar.build(point_cloud=numpy_point_cloud, timestamp=timestamp)

def read_one_msg(_db: sqlite3.Cursor):
    timestamp, binary_data = _db.execute(src.query.query_get_one_msg(8)).fetchone()
